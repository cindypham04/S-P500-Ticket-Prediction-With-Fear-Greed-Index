...
def main():
    ...
    # Use the dataset with Fear & Greed features
    input_file = "./dataset_creation/sp500_daily_features_with_fear_greed.csv"
    ...
    # Create sequential validation splits
    print("\nCreating Sequential Validation Splits")
    print("-" * 30)
    splits_info, test_df = create_sequential_validation_splits(
        input_file, 
    ...
    print(f"   Technical indicators: {len([col for col in feature_columns if not col.startswith('Lag_') and not any(idx in col for idx in ['Nikkei', 'Hang_Seng', 'All_Ordinaries', 'DAX', 'FTSE', 'NYSE', 'DJIA']) and not col.startswith('Fear_Greed') and not col.startswith('fg_class_')])}")
    ...
    print(f"   Fear & Greed features: {len([col for col in feature_columns if col.startswith('Fear_Greed') or col.startswith('fg_class_')])}")
