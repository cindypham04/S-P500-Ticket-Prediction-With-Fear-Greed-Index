...
def prepare_features_and_target(df):
    ...
    exclude_cols = ['Ticker', 'Date', 'Open', 'High', 'Low', 'Close', 'Volume', 'Return', 'Target', 'Fear_Greed_Classification']
    feature_cols = [col for col in df.columns if col not in exclude_cols]
    
    # Additional safety: exclude any remaining object/string columns
    numeric_cols = [col for col in feature_cols if df[col].dtype in ['int64', 'float64', 'bool']]
    if len(numeric_cols) < len(feature_cols):
        excluded_string_cols = [col for col in feature_cols if col not in numeric_cols]
        print(f"    Warning: Excluding {len(excluded_string_cols)} non-numeric columns: {excluded_string_cols[:5]}")
...
def main():
    ...
    if not os.path.exists(test_file):
        ...
        print("Will skip final test evaluation. You can run it later when test file is available.")
        print("Proceeding with sequential validation on available folds...\n")
        test_file = None
    ...
    if test_file and os.path.exists(test_file):
        ...
        print("\n⚠️  Skipping final test evaluation (test file not available)")
        print("Validation results only:")
        for model_name in validation_results.keys():
            print(f"\n{model_name}:")
            print(f"   Average Validation Accuracy:  {np.mean(validation_results[model_name]['accuracies']):.4f}")
            print(f"   Average Validation Precision: {np.mean(validation_results[model_name]['precisions']):.4f}")
            print(f"   Average Validation Recall:    {np.mean(validation_results[model_name]['recalls']):.4f}")
            print(f"   Average Validation F1-Score:  {np.mean(validation_results[model_name]['f1_scores']):.4f}")
            print(f"   Average Validation AUC:       {np.mean(validation_results[model_name]['aucs']):.4f}")
        test_results = {}
